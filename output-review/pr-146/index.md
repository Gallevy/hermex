...
26 cases · 4 changed · 0 invariant breach(es)

## Changed

| Case | Status | Proves |
| --- | --- | --- |
| [`comply-human-warn-only`](./comply-human-warn-only.html) | **changed** <span class="or-add">+2</span> <span class="or-del">−2</span> | Warn and info findings are reported but do not fail the build — verdict wording plus exit 0. |
| [`comply-all-rule-types`](./comply-all-rule-types.html) | **changed** <span class="or-add">+2</span> <span class="or-del">−2</span> | Every one of the nine rule types in one table, at three severities — the only case that renders require-engine-version, codeowners and both package-field shapes. |
| [`comply-all-rule-types-json`](./comply-all-rule-types-json.html) | **changed** <span class="or-add">+9</span> <span class="or-del">−9</span> | The machine-readable shape of every rule type: fieldPath and actualValue on package-field hits, installedRange/requiredRange on require-engine-version, matchedFiles on codeowners. Also where #95 is visible — the two codeowners entries are byte-identical apart from matchedFiles. |
| [`comply-overrides`](./comply-overrides.html) | **changed** <span class="or-add">+2</span> <span class="or-del">−2</span> | Repo-scoped overrides re-scope severities: one rule downgraded to warn, one switched off and gone from the table. |
