...
| [`repos/all-rule-types/`](./repos/all-rule-types/README.md) | All eleven rule types firing at once, at three severities. The only fixture that renders a `max-file-size`, `require-engine-version`, `codeowners`, `require-repo-name-match`, or package-field row, so it is the one that covers those renderers at all. Its CODEOWNERS deliberately leaves one file unowned and gives another to a team outside `requiredOwners` — which is how it surfaced #95. |
